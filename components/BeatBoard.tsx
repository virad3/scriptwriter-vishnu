...
const BeatBoard: React.FC<BeatBoardProps> = ({ beats, setBeats }) => {
  ...
  return (
    <div className="p-8 h-full overflow-y-auto bg-[#1e1e1e]">
        <div className="flex justify-between items-center mb-8">
            ...
            <div className="flex gap-2">
                <input 
                    ...
                    className="bg-gray-800 text-sm px-4 py-2 rounded-lg border border-gray-700 text-white w-64 focus:outline-none focus:border-blue-500"
                />
                <button 
        ...
        <div className="grid grid-cols-4 gap-6 min-w-[1000px]">
            {acts.map(act => (
                <div key={act} className="flex flex-col gap-4">
                    <div className="text-gray-400 font-bold uppercase text-xs tracking-widest border-b border-gray-700 pb-2 mb-2">
                        Act {act.toUpperCase()}
                    </div>
                    {beats.filter(b => b.act === act).map(beat => (
                        <div key={beat.id} className="bg-gray-800 p-4 rounded-lg border border-gray-700 hover:border-gray-500 cursor-grab group relative shadow-md">
                            <div className="w-full h-1 absolute top-0 left-0 rounded-t-lg" style={{ background: beat.color || '#4b5563' }}></div>
                            <div className="flex justify-between items-start mb-2 mt-2">
                                <h4 className="font-bold text-gray-200 text-sm">{beat.title}</h4>
                                <button 
                            ...
                            <p className="text-gray-400 text-xs leading-relaxed">{beat.synopsis}</p>
                        </div>
                    ))}
                    <button 
                        onClick={() => {
                        ...
                        className="w-full py-3 border-2 border-dashed border-gray-800 rounded-lg text-gray-600 hover:border-gray-600 hover:text-gray-400 flex justify-center items-center transition-all"
                    >
                        <Plus size={20} />
                    </button>
                </div>
